...
public class BoardMgr_1 {
	...
	public Vector getBoardList(String keyField, String keyWord) {
		  ...
		  try {	 
			 ...
			 if(keyWord.equals("null") || keyWord.equals("")){
			   strQuery = "select * from board_1 order by pos asc";
			 }else{
			   strQuery = "select * from board_1 where "
			   +keyField+" like '%" + keyWord +"%' order by pos asc";
			 }
			rs = stmt.executeQuery(strQuery);
			while (rs.next()) {
			BoardBean_1 tempBean = new BoardBean_1();

			tempBean.setNum(rs.getInt("num1"));
	...
	public void insertBoard(BoardBean_1 boardBean){
		  Connection con = null;
		  ...
		  try {
			con = pool.getConnection();
			String strQuery = "insert into board_1" 
					+ "(num1, name,email,homepage,subject,content,pos,depth,regdate,pass,count,ip)" 
				        //다음 값 반환
					+ " values(num1.nextval,?,?,?,?,?,?,?,sysdate,?,?,?)"; 
	...
	public void upPos(){ 
		  ...
		  try{
			con = pool.getConnection();
			stmt=con.createStatement();
			String strQuery="update board_1 set pos=pos+1";
	...
	public BoardBean_1 getBoard(int num) { 
		  Connection con = null;
		  PreparedStatement pstmt = null;
		  ResultSet rs = null;
		  BoardBean_1 tempBean = new BoardBean_1();
		  ...
		  try {
			con = pool.getConnection();
			String strQuery = "select * from board_1 where num1 = ?";
			pstmt = con.prepareStatement(strQuery);
			pstmt.setInt(1,num);
		    ...
		    while (rs.next()) {
			  ...
			  System.out.println("select * from board_1 where num1 Exception" + ex);
		  }finally{
	...
	public void upCount(int num) { 
		  ...
		  try{
			con = pool.getConnection();
			String strQuery="update board_1 set count=count+1 where num1= ?";
			pstmt = con.prepareStatement(strQuery);
			pstmt.setInt(1,num);
			pstmt.executeUpdate();
		  }catch(Exception ex) {
			System.out.println("update board_1 set Exception" + ex);
	      }finally{
	        pool.freeConnection(con,pstmt);		  
		  }
	}
	...
	public void deleteBoard(int num) {  
		  ...
		  try{
			con = pool.getConnection();
			String strQuery = "delete from board_1 where num1 = ?";
			pstmt = con.prepareStatement(strQuery);
			pstmt.setInt(1,num);
			pstmt.executeUpdate();
		  }catch(Exception ex) {
		      System.out.println("delete from board_1 Exception" + ex);
		  }finally{
	        pool.freeConnection(con,pstmt);		  
		  }
	}

	public void updateBoard(BoardBean_1 boardBean){
		  Connection con = null;
		  PreparedStatement pstmt = null;
		
	      try{
			con = pool.getConnection();
			String strQuery = "update board_1 set "
							+ "subject=?,content=?" 
							+ "where num1=?";

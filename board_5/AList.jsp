...
function list(){
	document.list.action="../board_5/AList.jsp";
 	document.list.submit();
 }

 function read(value){
	document.read.action="../board_5/ARead.jsp";
...
<% if(totalRecord !=0){ %> 
<% if (nowBlock > 0) {%> 
<a href="../board_5/AList.jsp?nowBlock=<%=nowBlock - 1 %>&page=<%=((nowBlock - 1) * pagePerBlock)%>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">
이전 <%=pagePerBlock %> 개</a>
<%}%> 
:::
<%
for (int i = 0; i < pagePerBlock; i++) { %>
<a href="../board_5/AList.jsp?nowBlock=<%=nowBlock %>&page=<%=(nowBlock*pagePerBlock) + i %>&keyField=<%=keyField%>&keyWord=<%=keyWord%>">
...
<%} %>
  ...
  <td align=right> 
   <a href="../board_5/APost.jsp">[등록하기]</a><a href="javascript:list()">[처음으로]</a> 
  </td>
 </tr>
</table>

	<br>
	<form action="../board_5/AList.jsp" name="search" method="post">
